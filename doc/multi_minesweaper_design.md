...
本ドキュメントでは、Vue 3 + Pinia + Laravel + Inertia で実装するオンライン協力型マインスイーパーの仕様・設計を記述します。  
...
| id (PK)      | BIGINT    | 自動採番 ID                                                      |
| room_id (FK) | BIGINT    | rooms テーブルの外部キー                                         |
| board_state  | JSON      | ボードの状態 (地雷配置、開閉情報など)                            |
| turn_info    | JSON      | ターン制に関する情報（今のプレイヤー、残りフラグアクションなど） |
